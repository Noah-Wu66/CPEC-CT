...
import { BlobFileStore as BlobFile } from "@/lib/ai/server/store";
...
const SANDBOX_ROOT = "/vercel/sandbox/ai-studio";
...
async function installSandboxPackages(sandbox, packages, errorMessage, { optional = false } = {}) {
  ...
  if (installCommand.exitCode === 0 || optional) {
    if (installCommand.exitCode !== 0) {
      ...
      logWarn("vercel-sandbox", "optional package install failed", null, {
