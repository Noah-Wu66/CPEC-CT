import { apiJson } from "@/lib/ai/client/apiClient";
...
function resolveDirectChatEndpoint(modelId) {
  ...
  if (config?.isImageGen) return "/api/ai/chat/image";
  return "/api/ai/chat/bailian";
}
...
async function compressHistory(messages) {
  const data = await apiJson("/api/ai/chat/compress", {
    method: "POST",
    body: { messages },
  });
  return data.summary;
