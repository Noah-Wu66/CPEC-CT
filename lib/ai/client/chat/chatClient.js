...
import { stripProviderControlText } from "@/lib/ai/shared/providerText";
import { normalizeWebSearchSettings } from "@/lib/ai/shared/webSearch";
import { normalizeWebBrowsingIdentifier } from "@/lib/ai/shared/webBrowsing";
...
const ALLOWED_TIMELINE_KINDS = new Set(["thought", "search", "reader", "tool", "approval", "upload", "parse", "planner", "writer"]);
...
function normalizePersistedCitations(citations) {
  ...
  const next = citations
    ...
    .map((item) => {
      ...
      if (typeof item.favicon === "string" && item.favicon) {
    ...
    const next = parts
      ...
      .map((part) => {
        ...
        if (part.inlineData && typeof part.inlineData === "object" && isNonEmptyString(part.inlineData.url)) {
          ...
          if (fileId) entry.inlineData.fileId = fileId;
        }
        ...
        if (part.fileData && typeof part.fileData === "object") {
          ...
          if (
            ...
            if (fileId) entry.fileData.fileId = fileId;
          }
        }
...
function hasMeaningfulModelProgress(message, { content, parts, thinkingTimeline, citations, tools, artifacts, providerState } = {}) {
  ...
  if (providerState || message.providerState) {
    return true;
  }
...
function normalizePersistedMessage(message) {
  ...
  if (!hasMeaningfulModelProgress(message, { content, parts, thinkingTimeline, citations, tools, artifacts, providerState })) {
...
function resolveDirectChatEndpoint() {
  return "/api/ai/chat";
}
...
export function buildChatConfig({
  ...
  if (Array.isArray(images) && images.length > 0) {
    cfg.images = images
      .filter((item) => typeof item?.fileId === "string" && item.fileId && typeof item?.url === "string" && item.url)
      .map((item) => ({
        fileId: item.fileId,
        url: item.url,
...
export async function runChat({
  ...
  const apiEndpoint = resolveDirectChatEndpoint();
  ...
  try {
    ...
    const applyEventPayload = (payload) => {
      ...
      try {
        ...
        if (data.type === "thought") {
          ...
          const delta = typeof data.content === "string" ? data.content : "";
          fullText += delta;
          displayedText = stripProviderControlText(fullText);
          if (!thinkingEnded) {
          ...
          const tool = data.tool && typeof data.tool === "object" ? data.tool : null;
          if (tool?.id) {
            const index = tools.findIndex((item) => item?.id === tool.id);
            tools = index >= 0
              ? tools.map((item, itemIndex) => (itemIndex === index ? tool : item))
              : [...tools, tool];
            scheduleFlush();
          }
    ...
    if (!signal?.aborted && sawDone && convIdForSync) {
      (async () => {
        try {
          const convRes = await fetch(`/api/ai/conversations/${convIdForSync}`);
