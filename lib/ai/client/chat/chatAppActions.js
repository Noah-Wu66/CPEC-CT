...
import { buildChatConfig, buildPersistedConversationMessages, runChat, unlockCompletionSound } from "@/lib/ai/client/chat/chatClient";
import { isDataImageUrl, isHttpUrl } from "@/lib/ai/shared/messageImage";
import { createAttachmentDescriptor } from "@/lib/ai/shared/attachments";
import { isImageAttachment } from "@/lib/ai/shared/messageAttachments";
import {
...
export function createChatAppActions({
  ...
  const syncConversationMessages = async (nextMessages) => {
    ...
    try {
      ...
      toast.error(error?.message || "保存消息失败");
    }
  };
