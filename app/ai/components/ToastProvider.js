...
const TOAST_TYPES = {
  success: {
    icon: CheckCircle,
    bg: "toast-success",
    iconColor: "text-[var(--oa-green)]",
  },
  error: {
    icon: AlertCircle,
    bg: "toast-error",
    iconColor: "text-[var(--oa-red)]",
  },
  warning: {
    icon: AlertTriangle,
    bg: "toast-warning",
    iconColor: "text-[var(--oa-gold)]",
  },
  info: {
    icon: Info,
    bg: "toast-info",
    iconColor: "text-[var(--oa-blue)]",
  },
};
...
function ToastItem({ toast, onDismiss }) {
  ...
  return (
    <motion.div
      ...
      className={`
        flex items-center gap-2.5 px-4 py-2.5 rounded-full border shadow-[var(--oa-shadow-soft)] backdrop-blur-sm
        ${config.bg}
